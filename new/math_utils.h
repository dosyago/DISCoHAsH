...
namespace math_utils {
  double calculate_entropy(const std::vector<uint64_t>& bit_position_changes, int iterations) {
    double entropy = 0.0;
    for (const auto &count : bit_position_changes) {
      double p = static_cast<double>(count) / iterations;
      if (p > 0.0) {
