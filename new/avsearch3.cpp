...
AvalancheStatistics avalanche_quality(uint64_t P, uint64_t G) {
  ...
  std::map<int, int> histogram;
  std::vector<uint64_t> bit_position_changes(64, 0);
  int sumBitChanges = 0;
  ...
  for (int i = 1; i <= iterations; i++) {
    ...
    uint64_t xor_result = state_prime ^ modified_state_prime;
    for (int pos = 0; pos < 64; ++pos) {
      if (xor_result & (1ULL << pos)) {
        bit_position_changes[pos]++;
      }
    }
  }
  ...
  double entropy = math_utils::calculate_entropy(bit_position_changes, iterations);

  return {P, G, zero_bits_percentage, average, stdev, histogram, bit_position_changes, entropy};
}
...
std::string generate_bit_position_histogram_string(const std::vector<uint64_t>& bit_position_changes, int iterations) {
  std::stringstream ss;
  ss << "Bit Position Changes:\n";
  for (size_t pos = 0; pos < bit_position_changes.size(); ++pos) {
    ss << "Bit " << std::setw(2) << pos << ": ";
    int barLength = bit_position_changes[pos] / (iterations/200);
    for (int i = 0; i < barLength; i++) ss << "#";
    if (barLength > 0) ss << " ";
    ss << "(" << bit_position_changes[pos] << ")\n";
  }
  return ss.str();
}

bool ranking_function(const AvalancheStatistics& a, const AvalancheStatistics& b) {
  double score_a = 20 * std::abs(a.mean - 32) + 60 * a.stddev + 6 * a.zero_bits_percentage - 100 * a.entropy;
  double score_b = 20 * std::abs(b.mean - 32) + 60 * b.stddev + 6 * b.zero_bits_percentage - 100 * b.entropy;
