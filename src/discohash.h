...
class DiscoHash {
    private:
        ...
        std::array<uint64_t, STATE64> ds;
        std::array<uint8_t, STATE> ds8;
        ...
        void round( const std::array<uint64_t, STATE64>& m64, const std::array<uint8_t, STATE>& m8, int len ) {
            int index = 0;
            int sindex = 0;
            int Len = len >> 3;
