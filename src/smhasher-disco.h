...
inline void BEBB4185_64(const void* key, int len, uint32_t seed, void* out) {
  ...
  int numOfFullChunks = len / sizeof(uint64_t);
  int remainderBytes = len % sizeof(uint64_t);

  std::vector<uint64_t> key64(numOfFullChunks);
  if ( numOfFullChunks > 0 ) {
    memcpy(key64.data(), key8Arr, numOfFullChunks * sizeof(uint64_t));
  }
